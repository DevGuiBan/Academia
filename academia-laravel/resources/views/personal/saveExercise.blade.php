...
<div class="flex flex-col space w-full">
    ...
    <div class="flex flex-col items-center w-full max-w-md border border-gray-500 p-6">
        <form action={{route('personal.updateExercise',['id'=>$exercise->id,'personal_id'=>session('user_id')])}} method="post" class="w-full mt-6">
            @csrf
            @method('PUT')
            <label for="nome" class="text-gray-500">Nome do Exercicio</label>
            <input type="text" name="nome" id="nome" required class="w-full p-2 mt-1 mb-4 bg-gray-800 text-white border border-gray-600 rounded" value="{{ old('nome', $exercise->nome) }}">

            <label for="repeticoes" class="text-gray-500">Quantidade de Repetições</label>
            <input type="text" name="repeticoes" id="repeticoes" required class="w-full p-2 mt-1 mb-4 bg-gray-800 text-white border border-gray-600 rounded"  value="{{ old('repeticoes', $exercise->quantidade_de_repeticoes) }}">

            <label for="link" class="text-gray-500">Link de visualização</label>
            <input type="text" name="link" id="link" required class="w-full p-2 mt-1 mb-4 bg-gray-800 text-white border border-gray-600 rounded" value="{{ old('link', $exercise->link_de_visualizacao) }}">
    ...
    @else
    <div class="flex flex-col items-center w-full max-w-md border border-gray-500 p-6">
        <form action={{route('personal.createExercise')}} method="post" class="w-full mt-6">
            @csrf
            <label for="nome" class="text-gray-500">Nome do Exercicio</label>
