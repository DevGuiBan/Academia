...
<div class="flex flex-col items-center p-5 min-h-screen ml-[10rem]">
    ...
    <form action="{{ route('student.searchPersonal') }}" method="GET" class="flex space-x-3 mb-10">
        <input
            type="text"
            name="query"
    ...
    <div class="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        ...
        @else
            @if (!isset($query))
                <p class="text-gray-500">Digite o nome para buscar professores.</p>
            @else
                <p class="text-red-500 text-lg">Nenhum usuário encontrado para "{{ $query }}"</p>
            @endif
        @endif
    </div>
</div>
